#!/usr/bin/env python3
"""结合 CoverM dRep MAG TPM 与 MAG × ARO 矩阵，计算样本加权 AMR 丰度。"""
import argparse
import os
import socket
import sys
...
def main():
    ...
    default_matrix = os.path.join(matrix_dir, "mag_amr_gene_count_matrix.tsv")
    default_tpm = os.path.join(coverm_dir, "MAG_tpm.csv")

    parser = argparse.ArgumentParser(description="Calculate dRep MAG-weighted AMR abundance")
    parser.add_argument("--mag-amr-matrix", default=default_matrix)
    parser.add_argument("--mag-tpm", default=default_tpm)
    parser.add_argument("--out-dir", default=matrix_dir)
    args = parser.parse_args()

    os.makedirs(args.out_dir, exist_ok=True)
    log_file = os.path.join(args.out_dir, "MAG_weighted_AMR_validation.log")
    ...
    print("========== dRep MAG 加权 AMR 丰度 ==========")
    print(f"时间: {datetime.now()}\n")

    mag_amr = load_mag_matrix(args.mag_amr_matrix)
    mag_tpm = load_mag_tpm(args.mag_tpm)

    common_mags = mag_amr.index.intersection(mag_tpm.index)
    print(f"MAG × ARO: {mag_amr.shape[0]} MAG, {mag_amr.shape[1]} ARO")
    print(f"CoverM TPM: {mag_tpm.shape[0]} MAG, {mag_tpm.shape[1]} 样本")
    print(f"匹配 MAG: {len(common_mags)}")

    mag_amr = mag_amr.loc[common_mags]
    mag_tpm = mag_tpm.loc[common_mags]

    weighted = mag_tpm.T.dot(mag_amr)
    weighted.index.name = "sample"
    weighted.columns.name = "ARO"

    weighted_path = os.path.join(args.out_dir, "mag_weighted_amr_TPM.tsv")
    weighted.to_csv(weighted_path, sep="\t")
    print(f"\n加权 ARO 数: {weighted.shape[1]}")
    print(f"输出: {weighted_path}")

    send_notification(
        "dRep MAG加权AMR丰度通知",
        f"脚本: 9.5_MAG_weighted_amr_abundance.py\n"
        f"服务器: {socket.gethostname()}\n"
        f"匹配MAG: {len(common_mags)}, ARO数: {weighted.shape[1]}, 样本数: {weighted.shape[0]}\n"
        f"输出: {args.out_dir}",
    )
